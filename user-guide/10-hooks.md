...
Both `command` and `url` support `${VAR}` and `$VAR` expansion, including parameter-expansion modifiers (e.g. `${VAR:-default}`). Load-time vs runtime expansion and `env` map lookup order are defined by the Grok hook runner — this snapshot does not ship a separate custom-hooks reference.
